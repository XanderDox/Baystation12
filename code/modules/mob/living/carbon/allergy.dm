...
/mob/living/carbon/proc/start_allergy(allergy_severity)
	...
	if ((trait_flags & MILD_ALLERGY) && allergy_severity <= TRAIT_LEVEL_MINOR)
		return
	if (chem_doses[/datum/reagent/inaprovaline] || chem_doses[/datum/reagent/adrenaline])
		return
...
/mob/living/carbon/handle_allergy()
	...
	if ((trait_flags & MILD_ALLERGY) && ((!length(active_allergies)) || chem_doses[/datum/reagent/inaprovaline] >= 3))
